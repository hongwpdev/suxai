...
| 설명 | 선택된 정수장의 최근 1시간 수질 데이터를 반환한다 (5분 캐시) |
...
| 메서드 | `GET` |
| URL | `/api/waterFlow` |
| 설명 | 선택된 정수장의 유량 데이터를 반환한다 (5분 캐시) |
...
| 메서드 | `GET` |
| URL | `/api/analysis` |
| 설명 | 수질·유량 데이터를 AI가 분석한 리포트를 반환한다 (sujCode별 5분 캐시) |
...
| stDt | 필수 | 조회 시작일 | `2026-05-23` |
| stTm | 필수 | 조회 시작시간 | `00` |
| edDt | 필수 | 조회 종료일 | `2026-05-23` |
| edTm | 필수 | 조회 종료시간 | `24` |
